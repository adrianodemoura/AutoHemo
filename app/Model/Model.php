		...
		if (!isset($this->db))
		{
			...
			} catch (PDOException $e) 
			{
				switch($e->getCode())
				{
					case '1049':
					case '1045':
						array_push($this->erros, array('codigo'=>$e->getCode(),'mensagem'=>$e->getMessage()));
						break;
				}
			}
		}
	...
	public function find($tipo='all',$params=array())
	{
		...
		if (!empty($fields))
		{
			...
			foreach($fields as $_l => $_cmp)
			{
				...
				{
					unset($fields[$_l]);
					$tipo = $this->esquema[$_cmp]['type'];
					if (!in_array($tipo, array('habtm','virtual','hasmany')))
					{
						array_push($_fields,$this->name.'.'.$_cmp);
					}
				}
			}
			$fields = $_fields;
			...
			case 'all':
				if (empty($fields))
				{
					foreach($this->esquema as $_cmp => $_arrProp)
					{
						...
						if (!in_array($_arrProp['type'], array('habtm','virtual','hasmany')))
						{
							array_push($fields,$this->name.'.'.$_cmp);
						}
						if ($_arrProp['type']=='habtm') array_push($cHabtm, $_cmp);
						if ($_arrProp['type']=='hasmany') array_push($cHasmany, $_cmp);
					}
				}
				break;
			case 'first':
		...
		foreach($_data as $_l => $_arrCmps)
		{
			...
			// incrementando hasman, caso possua
			if ($tipo=='all' && !empty($cHasmany))
			{
				foreach($cHasmany as $_Hasmany)
				{
					require_once(APP.'Model/'.$_Hasmany.'.php');
					$Hasmany = new $_Hasmany();
					$prop = $this->esquema[$_Hasmany];
					$paramsHasmany['fields'] 	= isset($prop['fields']) ? $prop['fields'] : null;
					$paramsHasmany['order'] 	= isset($prop['order'])  ? $prop['order'] : null;
					$paramsHasmany['where'][$Hasmany->name.'.'.$prop['keyFk']]	= $_arrCmps[ucfirst($prop['keyFk'])];
					$dataHasmany = $Hasmany->find('all',$paramsHasmany);
					foreach($Hasmany->sqls as $_lH => $_arrSql)
					{
						array_push
						(
							$this->sqls, 
							array('sql'=>$_arrSql['sql'], 'ts'=>$_arrSql['ts'], 'li'=>$_arrSql['li'])
						);
					}
					foreach($dataHasmany as $_lH => $_arrHms)
					{
						$data[$_l][$_Hasmany][$_lH] = $_arrHms[$_Hasmany];
					}
				}
			}
		}
				...
				foreach($_arrCmps as $_cmp => $_vlr)
				{
					...
					if (!is_array($_vlr))
					{
						...
						if (in_array($p['type'], array('date','datetime')))
						{
							...
							if (!empty($v))
							{
								if (strpos($v,'/')) $n1	= explode('/', substr($v,0,10));
								if (strpos($v,'-')) $n1	= explode('-', substr($v,0,10));
	...
	public function save($data=array())
	{
		...
		foreach($this->data as $_l => $_arrMods)
		{
			foreach($_arrMods as $_l2 => $_arrCmps)
			{
				...
				// definindo o tipo da sql
				$sqlTi 	= 'INSERT';
				foreach($this->primaryKey as $_l3 => $_cmpId)
				{
					if (isset($_arrCmps[$_cmpId]) && $_arrCmps[$_cmpId]>0)
					{
						$sqlTi = 'UPDATE';
					}
				}

				// montando a sql, campo a campo
				foreach($_arrCmps as $_cmp => $_vlr)
				{
			...
			{
				case 'UPDATE':
					$sqls[$_l] = 'UPDATE '.$this->prefixo.$this->tabela.
						' SET '.$sqlUp.' WHERE '.$where.';';
					break;
				case 'INSERT':
					$sqls[$_l] = 'INSERT INTO '.$this->prefixo.$this->tabela.
						' ('.implode(',',$sqlInC).') VALUES ('.implode(',',$sqlInV).');';
					break;
			}
		}
